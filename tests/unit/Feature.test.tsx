...
describe("Feature (component)", () => {
  ...
  it("sets and advances the shared speaking order", () => {
    const room = createMockRoom();
    render(<Feature room={room} config={config} />);
    fireEvent.click(screen.getByRole("button", { name: "Set order" }));
    expect(screen.getByRole("heading", { name: "Now speaking: Ada" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Next speaker" }));
    expect(screen.getByRole("heading", { name: "Now speaking: Ben" })).toBeInTheDocument();
  });
});
